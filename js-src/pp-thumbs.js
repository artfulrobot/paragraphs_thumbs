  ...
  attach: function (context, settings) {
    ...
    jQuery('.paragraphs-add-more-submit', context).not('.pp-thumbs-processed').each(function() {
      ...
      if (bundle_name in settings.pp_thumbs) {
        ...
        var btn = jQuery('<img/>')
          .attr('src', settings.pp_thumbs[bundle_name])
          .attr('alt', 'Add ' + bundle_name)
          .addClass('pp-thumbs-img-btn');
        var ctr = jQuery('<div/>')
          .addClass('pp-thumbs-img-wrapper')
          .on('click', function() {
            jQuery(orig_btn).trigger('mousedown');
          });
        ctr
        .append(jQuery('<div/>').addClass('pp-thumbs-thumb').append(btn))
        .append(jQuery('<div/>').text('Add ' + bundle_name).addClass('pp-thumbs-name'));
